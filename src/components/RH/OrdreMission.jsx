...
export default function OrdreMission({ employe = {}, entreprise = {}, onClose }) {
  ...
  const handleGenerer = () => {
    genererDocumentRH('ordre_mission', employe, entreprise, form)
  }
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-dark-900">Ordre de mission</h2>
      ...
      <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
        <fieldset className="border border-dark-200 rounded-xl p-4">
          <legend className="text-sm font-semibold text-primary-600 px-2">Détails de la mission</legend>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {champ('Objet', 'objet', 'text', { required: true })}
            {champ('Destination', 'destination', 'text', { required: true })}
            {champ('Date de début', 'date_debut', 'date', { required: true })}
            {champ('Date de fin', 'date_fin', 'date', { required: true })}
          </div>
          {textarea('Motif', 'motif')}
          <div>
            <label className="input-label">Moyen de transport</label>
            <select name="moyen_transport" value={form.moyen_transport} onChange={handleChange} className="input select">
