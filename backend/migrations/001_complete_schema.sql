-- =============================================
-- KOLEYA ERP PME — Schema PostgreSQL Unifie v2.0
-- =============================================
...
CREATE TABLE entreprises (
    ...
    nrcc VARCHAR(100), logo TEXT, cachet TEXT, devise VARCHAR(10) DEFAULT 'FCFA',
...
CREATE TABLE refresh_tokens (
    ...
    token_hash VARCHAR(255) NOT NULL, expires_at TIMESTAMP NOT NULL, cree_le TIMESTAMP DEFAULT NOW()
);
...
-- 5. PRODUITS (AVANT vente_lignes et achat_lignes)
CREATE TABLE produits (
    ...
    fournisseur VARCHAR(255), emplacement VARCHAR(100),
    supprime_le TIMESTAMP, supprime_par UUID, cree_le TIMESTAMP DEFAULT NOW(), mis_a_jour_le TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE mouvements_stock (
    ...
    type_mouvement VARCHAR(30) NOT NULL, quantite INTEGER NOT NULL,
    quantite_avant INTEGER, quantite_apres INTEGER, prix_unitaire DECIMAL(15,2),
    motif TEXT, reference VARCHAR(100), utilisateur VARCHAR(255), cree_le TIMESTAMP DEFAULT NOW()
);
...
BEGIN
  ...
  IF p_type = 'devis' THEN SELECT prefixe_devis INTO v_prefixe FROM entreprises WHERE id = p_entreprise_id;
  ELSE SELECT prefixe_facture INTO v_prefixe FROM entreprises WHERE id = p_entreprise_id; END IF;
  IF v_prefixe IS NULL THEN v_prefixe := 'FAC'; END IF;
  INSERT INTO sequence_numeros (entreprise_id, type, annee, compteur) VALUES (p_entreprise_id, p_type, v_annee, 1)
  ON CONFLICT (entreprise_id, type, annee) DO UPDATE SET compteur = sequence_numeros.compteur + 1
...
VALUES ('Koleya Demo', '+237600000000', 'demo@koleya.cm', 'business', false, NULL) ON CONFLICT DO NOTHING;
...
SELECT id, 'Entreprise Kamga', '+237691234567', 'kamga@email.com', 'Bonanjo, Douala' FROM entreprises WHERE email = 'demo@koleya.cm';
INSERT INTO clients (entreprise_id, nom, telephone, email, adresse)
SELECT id, 'Boutique Ngo Biyick', '+237677890123', '', 'Marche Central, Douala' FROM entreprises WHERE email = 'demo@koleya.cm';
INSERT INTO clients (entreprise_id, nom, telephone, email, adresse)
SELECT id, 'SPAR Makeda', '+237655456789', 'info@makeeda.cm', 'Bonapriso, Douala' FROM entreprises WHERE email = 'demo@koleya.cm';

INSERT INTO produits (entreprise_id, nom, reference, categorie, stock, stock_min, prix_achat, prix_vente, fournisseur)
SELECT id, 'Papier A4 (ramette)', 'PAP-A4-001', 'Fournitures', 150, 20, 1500, 2500, 'Cameroun Papeterie' FROM entreprises WHERE email = 'demo@koleya.cm';
INSERT INTO produits (entreprise_id, nom, reference, categorie, stock, stock_min, prix_achat, prix_vente, fournisseur)
SELECT id, 'Cartouche HP', 'ENR-HP-001', 'Informatique', 8, 10, 12000, 18000, 'Tech Supply' FROM entreprises WHERE email = 'demo@koleya.cm';
INSERT INTO produits (entreprise_id, nom, reference, categorie, stock, stock_min, prix_achat, prix_vente, fournisseur)
SELECT id, 'Cle USB 32Go', 'USB-32-001', 'Informatique', 45, 15, 3000, 5500, 'Tech Supply' FROM entreprises WHERE email = 'demo@koleya.cm';

INSERT INTO employes (entreprise_id, nom, poste, salaire, date_embauche, telephone)
SELECT id, 'Mbarga Jean', 'Technicien', 150000, '2025-06-01', '+237699111222' FROM entreprises WHERE email = 'demo@koleya.cm';
INSERT INTO employes (entreprise_id, nom, poste, salaire, date_embauche, telephone)
SELECT id, 'Nkoulou Marie', 'Comptable', 200000, '2025-03-15', '+237677333444' FROM entreprises WHERE email = 'demo@koleya.cm';
